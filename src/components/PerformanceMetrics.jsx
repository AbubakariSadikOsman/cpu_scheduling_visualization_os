...
const PerformanceMetrics = ({ selectedAlgorithm, processes, executed }) => {
  ...
  return (
    <div className='w-[500px] bg-gray-100 px-4 py-4 rounded-xl'>
      <h2 className='text-[1.4rem] mb-3 font-bold text-center'>Performance Metrics <span className='text-[0.7rem] border-b-purple-500 border-b-2'>{isNonPreemptive ? 'Non-Preemptive' : 'Preemptive'}</span></h2>
      <table className="w-full border-collapse border border-gray-300 text-center">
        <thead className={isNonPreemptive ? 'text-[0.82rem]' : 'text-[0.72rem]' }>
            <tr className="bg-gray-200">
                <th className="border border-gray-300 px-4 py-[0.67rem]">Process ID</th>
                  {isNonPreemptive && (
                    <th className="border border-gray-300 px-4 py-[0.67rem]">Waiting/Response Time</th>
                  )}
                  {!isNonPreemptive && (
                    <>
                      <th className="border border-gray-300 px-4 py-[0.67rem]">Waiting Time</th>
                      <th className="border border-gray-300 px-4 py-[0.67rem]">Response Time</th>
                    </>
                  )}
